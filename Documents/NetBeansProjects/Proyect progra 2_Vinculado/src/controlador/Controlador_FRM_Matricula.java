        ...
        if (e.getActionCommand()
            ...
            if (FRM_AlmacenarInformacion.numeroSeleccionado == 3)// se agrega a XML
            {
                if (archivo_Matricula_XML.consultarInformacionDelXml(frm_matricula.devolverCodigo())) {
                    ...
                    String opcion = JOptionPane.showInputDialog(null, "No se permite eliminar ninguna matricula, elija una opcion:/n 1)Eliminar definitivamente / n2)Cancelar");
                    
                    if(opcion.equalsIgnoreCase("1")||opcion.equalsIgnoreCase("uno"))
                    {
                        archivo_Matricula_XML.eliminarInformacionDelXml(opcion);
                    }
                    else
                    {
                        frm_matricula.limpiarTabla();
